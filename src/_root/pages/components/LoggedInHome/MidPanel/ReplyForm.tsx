...
const ReplyForm: React.FC<{ parentPostId: string; space: string; postOwnerUsername: string }> = ({ 
    parentPostId, 
    space, 
    postOwnerUsername 
  }) => {
    ...
    const handleReplySubmit = async () => {
      ...
      try {
        // Add the reply to the "replies" subcollection of the selected post
    ...
    const handleContentChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
      // Allow changes as long as the length is <= maxLength
    ...
    const adjustTextareaHeight = () => {
      if (textareaRef.current) {
        // Reset the height to 'auto' to shrink and expand dynamically
        textareaRef.current.style.height = 'auto';
        // Set the height based on scrollHeight (total height needed for content)
      ...
      <div className="bg-white px-4 pt-4 pb-3 rounded-lg border border-slate-200 focus-within:ring-2 focus-within:ring-primary-500">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <span>to</span>
            <span className="text-gray-500"> @{postOwnerUsername}</span>
          </div>
        </div>
        ...
        <textarea
          ...
          style={{
            ...
            overflow: 'hidden', // Prevents scrolling within the textarea
          }}
        />
